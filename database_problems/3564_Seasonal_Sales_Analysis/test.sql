...
*/
-- Final SQL command --
with sales_season as (select case
    ...
    end as season, p.category, sum(quantity) total_quantity, sum(quantity*price) total_revenue
from sales s 
left join products p
on s.product_id = p.product_id
group by season, p.category), category_rank as (
select *, 
    dense_rank() over (partition by season order by total_quantity desc, total_revenue desc) as crank
    from sales_season
)
select season, category, total_quantity, total_revenue from category_rank
where crank = 1
order by season;