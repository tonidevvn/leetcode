...
*/
-- Final SQL command --
with T1 as (select count(session_id) as rating_count, book_id, min(session_rating) min_rate, max(session_rating) max_rate from reading_sessions 
group by book_id 
having min_rate <= 2 and max_rate >= 4 and rating_count >= 5), T2 as (select count(session_id) as extreme_rating_count, book_id from reading_sessions where session_rating <= 2 or session_rating >= 4 group by book_id), T3 as ( 
select T1.book_id, title, author, genre, pages, (max_rate - min_rate) as rating_spread, round(extreme_rating_count / rating_count, 2) as polarization_score from T1, T2, books
where T1.book_id = T2.book_id and T1.book_id = books.book_id)
select * from T3
