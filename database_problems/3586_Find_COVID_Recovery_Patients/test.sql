...
*/
-- Final SQL command --
with positive_cases as (select patient_id, min(test_date) as first_positive
from covid_tests 
where result = 'Positive'
group by patient_id), negative_cases as(
select ct.patient_id, min(test_date) as first_negative, first_positive
from covid_tests ct, positive_cases pc
where result = 'Negative' and ct.patient_id = pc.patient_id and test_date >= first_positive
group by patient_id)
select p.patient_id, patient_name, age, DATEDIFF(first_negative, first_positive) as recovery_time 
from patients p, negative_cases nc
where p.patient_id = nc.patient_id
