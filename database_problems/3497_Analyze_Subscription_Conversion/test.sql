...
*/
-- Final SQL command --
with trial_period as (select distinct user_id, avg(activity_duration) over (partition by user_id) as trial_avg_duration 
from UserActivity
where activity_type = 'free_trial'
order by user_id), paid_period as (select distinct user_id, avg(activity_duration) over (partition by user_id) as paid_avg_duration  
from UserActivity
where activity_type = 'paid'
order by user_id)
select paid_period.user_id, round(trial_avg_duration, 2) as trial_avg_duration, round(paid_avg_duration, 2) as paid_avg_duration
from paid_period
left join trial_period
on trial_period.user_id = paid_period.user_id;